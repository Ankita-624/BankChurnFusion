...
function renderChart(churn, nonChurn){
  ...
  chart=new Chart(ctx,{
    type:"doughnut",
    data:{labels:["Churned","Retained"],datasets:[{data:[churn,nonChurn],backgroundColor:["#ef4444","#3b82f6"]}]},
    options:{responsive:true,maintainAspectRatio:false,devicePixelRatio:dpr,plugins:{legend:{position:"top"}},cutout:"60%"}
  });
...
/* ---------------- state ---------------- */
let baselineCustomers=[];   // from scored_customers.csv
...
async function loadBaseline(){
  ...
  if(csv){
    ...
    baselineCustomers = rows.map(r=>({
      id:r["CustomerID"], pFused:toNum(r["P_Fused"]), churn:String(r["Churn_Predicted"]).trim()==="1"
...
function renderTopTable(customers, tbody){
  ...
  tbody.innerHTML=top.map(c=>`<tr><td>${c.id}</td><td>${fmtProb(c.pFused)}</td><td>${c.churn?"Yes":"No"}</td></tr>`).join("");
}

/* ---------------- download Top-Risk CSV ---------------- */
function downloadTopRisk(customers){
  const rows = customers.filter(c=>c.pFused>=threshold)
                        .sort((a,b)=>b.pFused-a.pFused)
                        .map(c=>({CustomerID:c.id,P_Fused:c.pFused,Churn_Predicted:c.churn?1:0}));
...
async function uploadAndScore(){
  ...
  try{
    ...
    uploadedCustomers = data.rows.map(r=>({
      id:r.CustomerID, pFused:toNum(r.P_Fused), churn: (r.Churn_Predicted===1)
    }));
